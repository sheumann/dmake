...
int
Make(CELLPTR cp, CELLPTR setdirroot)/*
	 ...
	 FREE(cname); 			/* cname == cmnd at this point. */
      ...
      if( group )
         Append_line( cmnd, TRUE, tmpfile, cp->CE_NAME, trace, 0 );
			...
			int kludge_int1, kludge_int2;			/* kludge variables for below */
...
/* this section is kludge for other commented out part */
         kludge_int1 = (l_attr&A_IGNORE) ? 1:0;
		   kludge_int2 = (rp->st_next == NIL(STRING)) ? 1:0;
/*	 		rval=Do_cmnd(cmnd,FALSE,do_it,cp, kludge_int1, FALSE, kludge_int2); */
  	 		rval=Do_cmnd(cmnd,FALSE,do_it,cp, kludge_int1, shell, kludge_int2); 
/* end kludge */
...
PUBLIC void
Print_cmnd(char *cmnd, int echo, int map )/*
================================
...
int
Push_dir(char *dir, char *name, int ignore )/*
===============================
...
static void
_set_recipe( char *target, int ind )/*
============================
