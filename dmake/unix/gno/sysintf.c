...
--
--	DMAKE SPECIFIC:
--		seek_arch()   -- from unix/arlib.c
--		touch_arch()  -- from unix/arlib.c
--		void_lcache() -- from unix/arlib.c
--		runargv()     -- from unix/runargv.c
--		STAT()
--		Remove_prq()
--
...
--		utime()
--		time()    -- orcalib
--		getenv()  -- lenviron, orcalib
--		putenv()  -- lenviron
--		getcwd()
--		signal()  -- libgno, orcalib
--		chdir()   -- lbsd
--		tempnam() 
-- 
-- AUTHOR
	...
	char **member)
{
   ...
   if (lib != NIL(char))
      ...
      return( utime(name, NIL(struct utimbuf)) );
}



PUBLIC void
Void_lib_cache( char *lib_name, char *member_name )/*
=========================================
	...
	int	  last)
{
   ...
   if( Max_proc == 1 ) Wait_for_completion = TRUE;
	...
	i = runargv(target, ignore, group, last, shell, cmd);  
...
PUBLIC char **
Pack_argv( int group, int shell, char *cmd )
{
...
*/
PUBLIC int
Write_env_string(char *ename, char *value)
{
#if !defined(GNO)
   extern int putenv();
#endif
...
#if defined(__GNUC__) || defined(__ORCAC__)
extern char **environ;
#endif

PUBLIC void
ReadEnvironment(void)
{
   char **rsave;
...
#if defined(DEBUG)
	if(getenv("CHECKSTACK")) printf("dmake stack usage = %d bytes\n",
	   end_stack_check());
#endif
...
*/
PUBLIC FILE*
Get_temp(char **path, char *suff, int op)
{
#if !defined (GNO)
   extern char *tempnam();
#endif
...
*/
PUBLIC FILE *
Start_temp( char *suffix, CELLPTR cp, char **fname )
{
   FILE	       *fp;
   char        *tmpname;
   char	       *name;
...
*/
PUBLIC void
Open_temp_error( char *tmpname, char *name )
{
...
*/
PUBLIC void
Link_temp( CELLPTR cp, FILE *fp, char *fname )
{
   FILELISTPTR new;
...
*/
PUBLIC void
Close_temp(CELLPTR cp, FILE *file)
{
   FILELISTPTR fl;
   if( cp == NIL(CELL) ) cp = Root;
...
PUBLIC void
Handle_result(int status, int ignore, int abort_flg, CELLPTR target)
{
